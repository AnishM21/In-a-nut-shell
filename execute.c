...
extern char pipe_arguments[100][1000];
extern char hist_file_loc[256];
...
void execute_piping()
{
    // Delimiting with space and tabs.
    // char temp_command[50][100];
    // int temp_com_count = 0;
    // char delim[4] = {' \t'};

    // char* t_arg = strtok(pipe_arg, delim);
    // while (t_arg != NULL)
    // {
    ...
    //     temp_com_count++;
    // }
    printf("Entered execute-piping func\n");
    // Checking the commands to see what the user wants to execute
    if (strcmp(temp_command[0], "cd") == 0)
    {
        if (temp_com_count > 2)
        {
            printf("Too many arguments passed for cd command\n");
        ...
        else if (temp_com_count == 1) // This means command entered was `cd` and should show user pseudo-home directory(~)
        {
            chdir(pseudo_dir);
            getcwd(cur_dir, 1000);
        }

        else if (strcmp(temp_command[1], ".") == 0)
        {
            changePrevDir(prevDir); // NEED TO WRITE THIS FUNCTION
            pwd();
        }
        ...
        {
            checkDir(temp_command[1], cur_dir, prevDir);
        }
        // printf("Previous directory-%s Current Directory-%s\n", prevDir, cur_dir);
    }

    // pwd command
    else if (strcmp(temp_command[0], "pwd") == 0)
    {
        pwd();
    }

    // echo command
    // else if (strcmp(temp_command[0], "echo") == 0)
    // {
    //     echo(temp_command, temp_com_count);
    // }

    // history command
    else if (strcmp(temp_command[0], "history") == 0)
    {
        if (temp_com_count > 1)
        {
            printf("Too many arguments for history command\n");
    ...
    // ls command
    else if (strcmp(temp_command[0], "ls") == 0)
    {
        char dir_file_storer[20][1000];
        int a_flag = 0, l_flag = 0, x = 0;
        if (temp_com_count == 1)
        ...
        {
            for (int i = 1; i < temp_com_count; i++)
            {
                if (temp_command[i][0] == '-')
                {
                    if (temp_command[i][1] == 'a')
                    {
                        a_flag = 1;
                    }
                    else if (temp_command[i][1] == 'l')
                    {
                        l_flag = 1;
                    }
                    else
                ...
                else if (strcmp(temp_command[i], "-a") == 0)
                {
                    a_flag = 1;
                }

                else if (strcmp(temp_command[i], "-l") == 0)
                {
                    l_flag = 1;
                }

                else if (strcmp(temp_command[i], "-la") == 0 || strcmp(temp_command[i], "-al") == 0)
                {
                    a_flag = 1;
                    l_flag = 1;
                }
                ...
                {
                    strcpy(dir_file_storer[x], temp_command[i]);
                    x++;
                }
            }
        }

        if (strcmp(temp_command[1], ".") == 0 && temp_com_count == 2)
        {
            plain_ls(cur_dir);
        }

        else if (strcmp(temp_command[1], "..") == 0 && temp_com_count == 2)
        {
            char *temp_directory = malloc(1000 * sizeof(char));
            chdir("..");
            getcwd(temp_directory, 1000);
        ...
        else if (strcmp(temp_command[1], "-a") == 0 && temp_com_count == 2)
        {
            ls_a(cur_dir);
        }

        else if ((strcmp(temp_command[1], "-l") == 0) && temp_com_count == 2)
        {
            ls_l(cur_dir);
        }

        else if ((strcmp(temp_command[1], "-al") == 0 || strcmp(temp_command[1], "-la") == 0) && temp_com_count == 2)
        {
            ls_la_al(cur_dir);
        }
    ...
    else if (strcmp(temp_command[0], "discover") == 0)
    {
        if (temp_com_count == 1)
        {
            // printf("Check 1\n");
        ...
        else
        {
            ...
            for (int i = 1; i < comm_count; i++)
            {
                ...
                else if (strcmp(temp_command[i], "-d") == 0)
                {
                    d_flag = 1;
                    directory_encountered = true;
                }
                else if (strcmp(temp_command[i], "-f") == 0)
                {
                    f_flag = 1;
                    directory_encountered = true;
                }
                else
                {
                    if (!directory_encountered)
                    {
                        ...
                        {
                            strcpy(temp_command[i], pseudo_dir);
                        }
                        DIR *d = opendir(temp_command[i]);
                        if (d == NULL)
                        {
                            printf("The given input %s does not exist\n", command[i]);
                            illegal_dir = true;
                        ...
                        {
                            strcpy(temp_dir, temp_command[i]);
                            dir_flag = 1;
                        }
                    }
                    else
                    {
                        file_flag = 1;
                        strcpy(temp_file, temp_command[i]);
                    }
                }
            }
            // printf("%d %d %d %d\n", d_flag, f_flag, dir_flag, file_flag);
    ...
    // pinfo command
    else if (strcmp(temp_command[0], "pinfo") == 0)
    {
        if (temp_com_count == 1)
        {
            pinfo_blank();
        ...
        {
            int x = atoi(temp_command[1]);
            pinfo_pid(x);
        }
    }

    else if (strcmp(temp_command[0], "exit") == 0)
    {
        // updateToFile(hist_file_loc);
        exit(0);
    }

    else if (strcmp(temp_command[0], "jobs") == 0)
    {
        printf("Reached here\n");
        jobs();
    }

    else if (strcmp(temp_command[0], "sig") == 0)
    {
        sig_func();
    }

    else if (strcmp(temp_command[0], "fg") == 0)
    {
        fg();
    }

    else if (strcmp(temp_command[0], "bg") == 0)
    {
        bg2();
    }
    ...
    {
        printf("This is a fore/back process %d\n", temp_com_count);
        int e = 0;
        for (int i = 0; i < temp_com_count; i++)
        {
            ...
            {
                strcpy(bgProcess[e], temp_command[i]);
                e++;
                // printf("%s\n", bgProcess[i]);
            }
            // printf("Finished iteration %d\n", i+1);
        }
        if (strcmp(temp_command[temp_com_count - 1], "&") == 0)
        {
            return;
        }
        f2(bgProcess, e);
